...
class ShapeFixer:
    ...
    def __init__(self, data_dir):
        ...
        self.data_name_shape = namedtuple(
            'data_name_shape', ['var', 'shape', 'replace'])
        self.model_name_shape = namedtuple(
            'model_name_shape', ['var', 'shape'])
        ...
        for rp in raw_parameters:
            if rp[0] == 'feed_dict' or rp[1].name.strip().startswith('{'):
                ...
                if not feed_dict:
                    ...
                    data_dependency = sorted(
                        data_dependency, key=lambda d: d.start_index)
                    for i in [0, 2]:
                        ...
                        real_k_shape = resolve_shape(
                            real_k)
                        real_v_shape = resolve_shape(
                            real_v)

                        if k_var in visitor.graph:
                            k_var = visitor.graph[k_var][-1]
                            real_k_shape = resolve_shape(
                                eval(k_var.name, f_globals, f_locals))
                        ...
                        if v_var in visitor.graph:
                            ...
                            real_v_shape = resolve_shape(
                                eval(v_var.name, f_globals, f_locals))
                            replace = False
                        else:
                            replace = True
                        v_data = self.data_name_shape(
                            v_var, real_v_shape, replace)
                ...
                for k, v in feed_dict.items():
                    ...
                    if isinstance(real_v, float):
                        continue
                    real_k_shape = resolve_shape(real_k)
                    real_v_shape = resolve_shape(real_v)
                    if real_k_shape == [1]:
                        continue
        ...
        for _, funcs in visitor.lineno_function_call.items():
            for f in funcs:
                ...
                if splited_f[-1] in {'fit', 'evaluate', 'predict', 'train'}:
                    try:
                        var = eval(
                            ''.join(splited_f[:-1]), f_globals, f_locals)
                        ...
                        if var.__class__.__name__ in ['Model', 'Sequential', 'Estimator']:
                            ...
                            for i, (key, raw_param) in enumerate(visitor.func_key_raw_params[f]):
                                if i < 2 and key in [None, 'x', 'y']:
                                    ...
                                    params = split_tuple_or_list_string(
                                        raw_param.name)
                                    for param in params:
                            ...
                            for param in data_params:
                                if param in visitor.graph:
                                    ...
                                    real_param = eval(
                                        data_dependency[-1].name, f_globals, f_locals)
                                    real_param_shape = resolve_shape(
                                        real_param)
                                    if (data_dependency[-1],
                                            real_param_shape) not in data_in_out_name_shape:
                                        d = self.data_name_shape(
                                            data_dependency[-1], real_param_shape, False)
                                        data_in_out_name_shape.append(d)
                                else:
                                    real_param = eval(
                                        param.name, f_globals, f_locals)
                                    real_param_shape = resolve_shape(
                                        real_param)
                                    d = self.data_name_shape(
                                        param, real_param_shape, True)
                                    data_in_out_name_shape.append(d)
                    except Exception as e:
                        print(e)
    ...
    def parse_keras_model(self, model, model_in_out_name_shape, visitor):
        ...
        for _, function_calls in list(visitor.lineno_function_call.items())[::-1]:
            ...
            if finded_output_layer:
                break
        model_in_name_shape = self.model_name_shape(
            input_layer_function_call, list(model.input_shape))
        if model_in_name_shape not in model_in_out_name_shape:
            model_in_out_name_shape.append(model_in_name_shape)
        model_out_name_shape = self.model_name_shape(
            output_layer_function_call, list(model.output_shape))
    ...
    def _fix_shape_incompatibility(self, question_id, try_index):
        ...
        except_lineno = -1
        shape_error = False
        collect_shapes.init_shapes_collection(filenames=[os.path.join(self.data_dir, f'{file_name}.py')],
                                              lineno_varname=visitor.lineno_varname)
        with collect_shapes.collect():
            try:
                module = importlib.import_module(
                    f'{self.data_dir}.{file_name}')
                ...
                if shape_error:
                    ...
                    if tensorflow_or_keras == 'tensorflow':
                        ...
                        for except_func in except_function_keys:
                            ...
                            if splited_func[-1] in {'run', 'eval'}:
                                ...
                                if not self.fixed:
                                    ...
                                    end_time = time.time()
                                    self.time_dict['tf_reshape_tf_x_data'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    ...
                                    end_time = time.time()
                                    self.time_dict['tf_reshape_tf_y_data'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    self.fixed, source = fix_patterns.expand_tf_x_data_last_dim(
                                        source, except_lineno, data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['tf_expand_tf_x_data_last_dim'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    ...
                                    end_time = time.time()
                                    self.time_dict['tf_modify_tf_model_output_shape'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    self.fixed, source = fix_patterns.modify_tf_model_input_shape(
                                        source, visitor, data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['tf_modify_tf_model_input_shape'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    self.fixed, source = fix_patterns.expand_x_data_first_dim(
                                        source, except_lineno, data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['tf_expand_x_data_first_dim'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    ...
                                    end_time = time.time()
                                    self.time_dict['tf_modify_y_data_shape'].append(
                                        end_time-start_time)
                            ...
                            if hasattr(fix_patterns, function_name):
                                ...
                                fix_function = getattr(
                                    fix_patterns, function_name)
                                self.fixed, source = fix_function(
                                    source, except_func, visitor, context)
                                end_time = time.time()
                                self.time_dict['tf_others'].append(
                                    end_time - start_time)
                    else:
                        first_layer_function = self.parse_keras_first_layer_function(
                            visitor, f_globals, f_locals)
                        self.parse_kera_data(
                            visitor, source, f_globals, f_locals, data_in_out_name_shape)
                        model = self.parse_keras_model_var(
                            visitor, f_globals, f_locals)
                        ...
                        if hasattr(model, 'input_shape') and hasattr(model, 'output_shape'):
                            self.parse_keras_model(
                                model, model_in_out_name_shape, visitor)
                        else:
                            if not self.fixed:
                        ...
                        if self.fixed:
                            self.write_new_source_code(
                                source, question_id, try_index)
                            ...
                            if splited_func[-1] in {'fit', 'evaluate', 'predict', 'train'}:
                                ...
                                if not self.fixed:
                                    ...
                                    self.fixed, source = fix_patterns.transpose_x_data(
                                        source, except_lineno, data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['keras_transpose_x_data'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    self.fixed, source = fix_patterns.expand_x_data_first_dim(
                                        source, except_lineno, data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['keras_expand_x_data_first_dim'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    ...
                                    end_time = time.time()
                                    self.time_dict['keras_add_flatten_layer'].append(
                                        end_time-start_time)
                                if not self.fixed and model_in_out_name_shape:
                                    start_time = time.time()
                                    ...
                                    self.fixed, source = fix_patterns.modify_model_output_shape(
                                        ...
                                        data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['keras_modify_model_output_shape'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    self.fixed, source = fix_patterns.modify_y_data_shape(
                                        source, loss_function, data_in_out_name_shape, model_in_out_name_shape)
                                    end_time = time.time()
                                    self.time_dict['keras_modify_y_data_shape'].append(
                                        end_time-start_time)
                                if not self.fixed:
                                    start_time = time.time()
                                    for key, rp in visitor.func_key_raw_params[first_layer_function]:
                                    ...
                                    end_time = time.time()
                                    self.time_dict['keras_modify_model_input_shape'].append(
                                        end_time-start_time)
                    if self.fixed:
                        self.write_new_source_code(
                            source, question_id, try_index)
    ...
    def fix_shape_incompatibility(self, question_id, max_try_number):
        for i in range(max_try_number):
            t = threading.Thread(
                target=self._fix_shape_incompatibility, args=(question_id, i))
            t.start()
            t.join()
            if not self.fixed:
                break
...
if __name__ == '__main__':
    ...
    icse = ('ICSE2020ToRepair.xlsx', 'ICSE2020',
            f'time/time{sys.argv[2]}/ICSE2020AscendingOrderTime.xlsx')
    so = ('StackOverflow.xlsx', 'StackOverflow',
          f'time/time{sys.argv[2]}/StackOverflowAscendingOrderTime.xlsx')
    args = {'icse': icse, 'so': so}
    dataset = args[sys.argv[1]]
    question_ids = pd.read_excel(dataset[0])['question id']
    times = []
    for qi in tqdm(question_ids):
        start = time.time()
        shapeFixer = ShapeFixer(dataset[1])
        shapeFixer.fix_shape_incompatibility(qi, 5)
        end = time.time()
        times.append(end-start)
        merge_dicts(time_dict, shapeFixer.time_dict)
    data = pd.DataFrame({'question id': question_ids, 'time': times})
    print(time_dict)
    json.dump(time_dict, open(
        f'time_dict_{sys.argv[1]}.json', 'w', encoding='utf-8'))
